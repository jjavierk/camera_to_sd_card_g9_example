...
static void SkipCommentAndWhiteSpace(unsigned char *pImg, int buf_len, int *i)
{
	int saw_nl = 1;
...
static unsigned int ReadValue(unsigned char *pImg, int buf_len, int *i)
{
...
int WriteImageToFile_sd(char *ImageName, unsigned int W, unsigned int H, unsigned char PixelSize, unsigned char *OutBuffer, unsigned char imgFormat)
{
	sd_create_file();
	// switch_fs_t fs;
	// __FS_INIT(fs);
    ...
    // {
        ...
        // for(int i=0;i<steps;i++){
        //     #ifndef SILENT
	    //         progress_bar("\rWriting image ",i,steps);
	    //     #endif
        //     ret+=sd_write(OutBuffer +(CHUNK_SIZE*i), CHUNK_SIZE);
        // }
        // if(((W*H*PixelSize) % CHUNK_SIZE) != 0)
        //     ret+=sd_write(OutBuffer+(CHUNK_SIZE*steps) , ((W*H*PixelSize) % CHUNK_SIZE)*sizeof(unsigned char));
        ret+=sd_write(OutBuffer, (W*H*PixelSize));
    //}
