...
#define	CS_H()		spi_cs_high() //PORTB |= 0x08	/* Set MMC CS "high" */
#define CS_L()		spi_cs_low()//PORTB &= 0xF7	/* Set MMC CS "low" */


void init_spi()
{
	/* Init CS */
	spi_init(19000000, CPOL_LOW, CPHA_LOW);

	/* Init the rest of the SPI periph */
}
...
)
{
	...
	// for (uint32_t inx = 0; inx<bc; inx++)
	// 	data_tx[inx] = 0xFF;
	memset(data_tx, 0xFF, bc);
	spi_transfer(data_tx, buff, bc);
//	BYTE r;
//
//
//	DI_H();	/* Send 0xFF */
